"""
Dynoplan/db-CBS C++ optimizer bridge for constrained quadcopter6d Db-RRT paths.
...
from db.constrained_db_optimize_quadcopter6d import (
    OptimizedTrajectoryView3D,
    Quadcopter6DTrajOptResult,
    _extract_obstacle_arrays_3d,
    _static_boundary_velocity_feasibility_3d_numba,
)
...
def _dynoplan_static_obstacles(env):
    ...
    for obs in getattr(env, "obstacles", []):
        if hasattr(obs, "l") and hasattr(obs, "w") and hasattr(obs, "h"):
            obstacles.append(
                {
                    "type": "box",
                    "center": [float(obs.x), float(obs.y), float(obs.z)],
                    "size": [
                        float(obs.l) + 2.0 * obstacle_buffer,
...
def _write_problem_yaml(path: Path, planner, options: CppDynoplanQuadcopter6DOptimizerOptions, moving_obstacles):
    ...
    if goal.shape[0] == 3:
        goal = np.array([goal[0], goal[1], goal[2], 0.0, 0.0, 0.0], dtype=np.float64)

    static_obstacles = _dynoplan_static_obstacles(planner.env)
    lines = [
        "environment:\n",
        "  min: [0, 0, 0]\n",
        f"  max: {_yaml_number_list(env_size[:3])}\n",
    ]
    if static_obstacles:
...
def _write_cfg_yaml(path: Path, options: CppDynoplanQuadcopter6DOptimizerOptions, solver_id: int):
    data = {
...
def _run_cpp_optimizer(
    ...
    cfg_yaml: Path,
    result_yaml: Path,
    options: CppDynoplanQuadcopter6DOptimizerOptions,
):
    cmd = [
...
def _read_trajopt_trajectory(path: Path):
    if not path.exists():
        return {}, np.empty((0, 6), dtype=np.float64), np.empty((0, 3), dtype=np.float64)
    ...
    xs = np.asarray(states, dtype=np.float64) if states else np.empty((0, 6), dtype=np.float64)
    us = np.asarray(actions, dtype=np.float64) if actions else np.empty((0, 3), dtype=np.float64)
...
def _read_cpp_result(path: Path):
    ...
    if xs.shape[0] == 0:
        ...
        if xs.ndim == 1 and xs.size == 0:
            xs = np.empty((0, 6), dtype=np.float64)
        if us.ndim == 1 and us.size == 0:
            us = np.empty((0, 3), dtype=np.float64)
        summary.update(
            {
...
def _python_validate(planner, xs: np.ndarray, us: np.ndarray) -> bool:
    if xs.shape[0] == 0 or us.shape[0] != xs.shape[0] - 1:
        return False

    spheres, boxes = _extract_obstacle_arrays_3d(planner.env)
    feasible, _, _, _ = _static_boundary_velocity_feasibility_3d_numba(
        xs,
        spheres,
        boxes,
        np.asarray(planner.env.size[:3], dtype=np.float64),
        float(planner.agent.radius),
        float(getattr(planner.env, "obstacle_buffer", 0.0)),
        float(getattr(planner.env, "boundary_buffer", 0.0)),
        float(planner.agent.max_speed),
    )
    if not feasible:
        return False

    goal_dist = np.linalg.norm(xs[-1, :3] - np.asarray(planner.goal[:3], dtype=np.float64))
...
def optimize_dbrrt_quadcopter6d_path_with_cpp_dynoplan(
    planner,
    *,
    options: Optional[CppDynoplanQuadcopter6DOptimizerOptions] = None,
) -> Quadcopter6DTrajOptResult:
    if options is None:
        options = CppDynoplanQuadcopter6DOptimizerOptions()
    ...
    try:
        ...
        result = Quadcopter6DTrajOptResult(
