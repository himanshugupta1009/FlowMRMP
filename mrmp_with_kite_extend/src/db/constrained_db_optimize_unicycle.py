...
@njit(cache=True)
def _dynamic_constraint_penalty_and_derivatives_inplace_numba(
    x,
    stage_time,
    constraints,
    agent_radius,
    ...
    cost = 0.0
    metric_dim = min(distance_metric_state_size, 2)
    eps = 1e-9
...
def _running_cost_terms_inplace_fused_numba(
    ...
    dynamic_cost = _dynamic_constraint_penalty_and_derivatives_inplace_numba(
        x,
        stage_time,
        constraints,
        agent_radius,
...
class UnicycleTrajOptOptions:
    ...
    # These weights below are better for getting better quality paths.
    ...
    # These weights below are better for sticking close the dbRRT path and
    # to just ensure feasibility.
    state_tracking_weight: float = 0.05
    theta_tracking_weight: float = 0.02
    ...
    # theta_tracking_weight: float = 0.05
    # control_tracking_weight: float = 0.1
...
class _UnicycleRunningModel:
    def __init__(self, crocoddyl, *, dt: float, env, agent,
                 ...
                 roundoff_digits: int, circles: np.ndarray, rects: np.ndarray):
        ...
        self._dynamic_Lx = np.zeros(3, dtype=np.float64)
        self._dynamic_Lxx = np.zeros((3, 3), dtype=np.float64)
    ...
    def _running_cost_terms_inplace_legacy(self, x, u, Lx, Lu, Lxx, Luu, Lxu):
        ...
        dynamic_cost = _dynamic_constraint_penalty_and_derivatives_inplace_numba(
            ...
            self.agent.radius,
            self.dt,
            self.agent.distance_metric_state_size,
...
class _UnicycleTerminalModel:
    def __init__(self, crocoddyl, *, env, agent, goal_xy: np.ndarray,
                 ...
                 constraints, clearance_threshold: float, final_time: float,
                 dt: float, roundoff_digits: int,
...
def optimize_unicycle_warm_start(*,
    ...
    if feasible and constraints is not None and len(constraints) > 0:
        if not constraint_satisfaction_numba(
            ...
            agent.radius,
            dt,
            agent.distance_metric_state_size,
            clearance_threshold,
            1,
        ):
            feasible = False
...
def _retry_options(options: UnicycleTrajOptOptions):
    return (
        ...
        replace(
            ...
            dynamic_constraint_weight=max(options.dynamic_constraint_weight, 500.0),
            max_iter=max(options.max_iter, 100),
            retry_on_infeasible=False,
        ),
        replace(
...
def _warm_start_fallback_result(planner, highres_states, us_init, options, optimizer_result):
    ...
    if feasible and constraints is not None and len(constraints) > 0:
        feasible = constraint_satisfaction_numba(
            constraints,
            0,
            len(constraints),
            highres_states[1:],
            ...
            planner.agent.distance_metric_state_size,
            float(getattr(planner, "clearance_threshold", 0.0)),
            getattr(planner, "roundoff_digits", 1),
        )
