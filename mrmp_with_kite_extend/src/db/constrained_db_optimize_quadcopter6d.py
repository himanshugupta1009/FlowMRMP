...
@njit(cache=True)
def _dynamic_constraint_penalty_and_derivatives_3d_inplace_numba(
    x,
    stage_time,
    constraints,
    agent_radius,
    ...
    if len(constraints) == 0 or stage_time <= 0.0:
        return 0.0

    cost = 0.0
    metric_dim = min(distance_metric_state_size, 3)
    eps = 1e-9
    ...
    for i in range(len(constraints)):
        ...
        if stage_time < start_conflict_time or stage_time > end_conflict_time:
            continue

        pos_index = int(round((stage_time - start_conflict_time) / delta_t, roundoff_digits))
        if pos_index < 0:
            pos_index = 0
        elif pos_index >= collision_agent_position_array.shape[0]:
            pos_index = collision_agent_position_array.shape[0] - 1
        ...
        acc = 0.0
        if metric_dim >= 1:
            diff0 = x[0] - collision_agent_position_array[pos_index, 0]
            acc += diff0 * diff0
        if metric_dim >= 2:
            diff1 = x[1] - collision_agent_position_array[pos_index, 1]
            acc += diff1 * diff1
        if metric_dim >= 3:
            diff2 = x[2] - collision_agent_position_array[pos_index, 2]
            acc += diff2 * diff2

        dist = math.sqrt(acc)
        threshold = collision_agent_radius + agent_radius + clearance_threshold
        deficit = threshold - dist
        if deficit <= 0.0:
            continue

        cost += 0.5 * dynamic_constraint_weight * deficit * deficit
        if dist <= eps:
            continue
        ...
        if metric_dim >= 1:
            grad0 = diff0 / dist
            g[0] += -dynamic_constraint_weight * deficit * grad0
            H[0, 0] += dynamic_constraint_weight * grad0 * grad0
        if metric_dim >= 2:
            grad1 = diff1 / dist
            g[1] += -dynamic_constraint_weight * deficit * grad1
            H[0, 1] += dynamic_constraint_weight * grad0 * grad1
            H[1, 0] += dynamic_constraint_weight * grad1 * grad0
            H[1, 1] += dynamic_constraint_weight * grad1 * grad1
        if metric_dim >= 3:
            grad2 = diff2 / dist
            g[2] += -dynamic_constraint_weight * deficit * grad2
            H[0, 2] += dynamic_constraint_weight * grad0 * grad2
            H[2, 0] += dynamic_constraint_weight * grad2 * grad0
            H[1, 2] += dynamic_constraint_weight * grad1 * grad2
            H[2, 1] += dynamic_constraint_weight * grad2 * grad1
            H[2, 2] += dynamic_constraint_weight * grad2 * grad2
...
def _running_cost_terms_inplace_fused_numba(
    ...
    dynamic_cost = _dynamic_constraint_penalty_and_derivatives_3d_inplace_numba(
        x,
        stage_time,
        constraints,
        agent_radius,
...
class Quadcopter6DTrajOptOptions:
    ...
    dynamic_constraint_weight: float = 100.0
...
class _QuadcopterRunningModel:
    def __init__(self, crocoddyl, *, dt: float, env, agent,
                 ...
                 constraints, clearance_threshold: float, stage_time: float,
                 roundoff_digits: int, spheres: np.ndarray, boxes: np.ndarray,
        ...
        self.stage_time = float(stage_time)
        self.roundoff_digits = int(roundoff_digits)
        self.distance_metric_state_size = int(getattr(agent, "distance_metric_state_size", 3))
        ...
        self._dynamic_Lx = np.zeros(6, dtype=np.float64)
        self._dynamic_Lxx = np.zeros((6, 6), dtype=np.float64)
    ...
    def _running_cost_terms_inplace_legacy(self, x, u, Lx, Lu, Lxx, Luu, Lxu):
        ...
        dynamic_cost = _dynamic_constraint_penalty_and_derivatives_3d_inplace_numba(
            ...
            self.clearance_threshold,
            self.options.dynamic_constraint_weight,
...
class _QuadcopterTerminalModel:
    def __init__(self, crocoddyl, *, env, agent, goal_state: np.ndarray,
                 options: Quadcopter6DTrajOptOptions, constraints,
                 clearance_threshold: float, final_time: float, dt: float,
                 roundoff_digits: int, spheres: np.ndarray, boxes: np.ndarray,
                 obstacle_buffer: float):
        self.crocoddyl = crocoddyl
        self.state = crocoddyl.StateVector(6)
...
def optimize_quadcopter6d_warm_start(
    *,
    ...
    roundoff_digits = int(round(-math.log10(dt))) if dt < 1.0 else 0
    ...
    for stage_idx, (x_ref, u_ref) in enumerate(zip(xs_init[:-1], us_init)):
        raw_model = _QuadcopterRunningModel(
            ...
            stage_time=float(stage_idx + 1) * dt,
    ...
    terminal_model = _QuadcopterTerminalModel(
        ...
        constraints=constraints,
        clearance_threshold=clearance_threshold,
        final_time=float(us_init.shape[0]) * dt,
        dt=dt,
        roundoff_digits=roundoff_digits,
        spheres=spheres,
        boxes=boxes,
    ...
    feasible, fail_idx, violation_type, signed_dist = _static_boundary_velocity_feasibility_3d_numba(
        xs_out,
        spheres,
        boxes,
        env_size,
    ...
    if not feasible and verbose:
        ...
        if violation_type == 1:
            ...
            print(
                ...
                f"{fail_idx}, state={x}, signed_distance={signed_dist}, "
                f"required>={required_clearance}"
            )
        elif violation_type == 2:
            lower_bound = agent.radius + boundary_buffer
            upper_bounds = env_size - (agent.radius + boundary_buffer)
            print(
                "Optimization infeasible: boundary violation at index "
                f"{fail_idx}, state={x}, xyz_range=[{lower_bound}, {upper_bounds}]"
            )
        elif violation_type == 3:
            print(
                "Optimization infeasible: velocity bound violation at index "
                f"{fail_idx}, state={x}, max_speed={agent.max_speed}"
            )

    final_goal_dist = np.linalg.norm(xs_out[-1][:3] - goal_state[:3])
    if final_goal_dist > float(goal_radius):
        feasible = False
        if verbose:
            print(
                "Optimization infeasible: terminal goal violation, "
                f"goal_dist={final_goal_dist}, goal_radius={goal_radius}"
            )

    if feasible:
        if not constraint_satisfaction_numba(
            constraints,
            0,
            len(constraints),
            xs_out[1:],
            ...
            agent.radius,
            dt,
            int(getattr(agent, "distance_metric_state_size", 3)),
            clearance_threshold,
            roundoff_digits,
        ):
            feasible = False
            if verbose:
                print("Optimization infeasible: dynamic KCBS constraint violation")
