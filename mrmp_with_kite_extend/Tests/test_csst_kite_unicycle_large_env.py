...
from kinodynamic_TI_eb_csst import KiteCSST
...
for agent in agents:
    ...
    kd_tree = CircularAngleIndexNumba(edge_bundle.start_states[:, 2], ids=edge_ids)
...
translate_funcs = [agent.kd_tree_point_translate_function for agent in agents]
sort_edges_funcs = [agent.sort_kd_tree_edges for agent in agents]
get_eb_kd_tree_query_funcs = [agent.get_eb_kd_tree_query for agent in agents]

s = np.random.randint(0, 1000)
s = 823
print("RNG Seed: ", s)
print("Agent radius:", agents[0].radius)

kite_csst = KiteCSST(agents=agents,
...
planning_time = kite_csst.plan_path()
print("Planner reported time:", planning_time)
print("Path found:", kite_csst.path_found)
print("Path cost:", kite_csst.path_cost)
print("Scalar path cost:", kite_csst.path_scalar_cost)
print("Path time:", kite_csst.path_time)
print("First solution planning time:", kite_csst.first_solution_planning_time)
print("First solution path cost:", kite_csst.first_solution_per_agent_cost)
print("Nodes:", kite_csst._node_matrix.count)
print("Witnesses:", kite_csst._witness_matrix.count)

high_res_paths = kite_csst.get_high_resolution_path_numpy_array()
if kite_csst.path_found:
    result = check_high_resolution_paths_collision_free(
        high_res_paths,
        agents=kite_csst.agents,
        distance_metric_state_size=kite_csst.agents[0].distance_metric_state_size,
        dynamic_agent_clearance=kite_csst.dynamic_agent_clearance,
        roundoff_digits=kite_csst.roundoff_digits,
    )
    print("Collision free:", result["collision_free"])
    print("Collision times:", result["collision_times"])
    print("Agent path times:", kite_csst.get_agent_path_times(high_res_paths))
    path_rrt_nodes, states, controls, timesteps, costs = kite_csst.get_path()
    print("Path nodes:", len(path_rrt_nodes))
    print("Path timesteps:", timesteps)
else:
    print("Collision free: not checked because no path was found")

# Optional visualization, matching the cRRT test style.
# path_rrt_nodes, states, controls, timesteps, costs = kite_csst.get_path()
# tcol = ['y', 'c', 'b', 'g', 'b', 'b']
# pcol = ['xkcd:powder pink', 'xkcd:metallic blue', 'xkcd:pastel orange',
#         'xkcd:pastel blue', 'xkcd:terracotta', 'xkcd:purplish grey']
# mprint = MultiRRTPrinter(env, kite_csst, path_rrt_nodes, tcol, pcol, joint_states=True)
# mprint.print_rrt('./media/kite_csst_10agents_unicycle.png', print_tree=False)
# mprint.print_rrt_animation('./media/kite_csst_10agents_unicycle.gif',
