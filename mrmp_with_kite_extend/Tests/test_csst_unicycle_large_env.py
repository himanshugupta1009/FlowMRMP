...
planning_time = csst.plan_path()
print("Planner reported time:", planning_time)
print("Path found:", csst.path_found)
print("Path cost:", csst.path_cost)
print("Scalar path cost:", csst.path_scalar_cost)
print("Path time:", csst.path_time)
print("First solution planning time:", csst.first_solution_planning_time)
print("First solution path cost:", csst.first_solution_per_agent_cost)
print("Nodes:", csst._node_matrix.count)
print("Witnesses:", csst._witness_matrix.count)

high_res_paths = csst.get_high_resolution_path_numpy_array()
if csst.path_found:
    result = check_high_resolution_paths_collision_free(
        high_res_paths,
        agents=csst.agents,
        distance_metric_state_size=csst.agents[0].distance_metric_state_size,
        dynamic_agent_clearance=csst.dynamic_agent_clearance,
        roundoff_digits=csst.roundoff_digits,
    )
    print("Collision free:", result["collision_free"])
    print("Collision times:", result["collision_times"])
    path_rrt_nodes, states, controls, timesteps, costs = csst.get_path()
...
# mprint = MultiRRTPrinter(env, csst, path_rrt_nodes, tcol, pcol, joint_states=True)
# mprint.print_rrt('./media/csst_10agents_unicycle.png', print_tree=False)
# mprint.print_rrt_animation('./media/csst_10agents_unicycle.gif',
