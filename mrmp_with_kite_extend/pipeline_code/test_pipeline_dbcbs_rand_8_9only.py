...
class TestPipelineDbaRandom(TestPipeline):
    OBS_KEY = 'obs'
    STARTS_KEY = 'starts'
    GOALS_KEY = 'goals'
    ...
    def __init__(self, test_classes, agent_builders, test_rounds=10, num_agents=8, master_seed=42):
        ...
        """
        if num_agents not in [8]: raise Exception("Use 8 agents")

        path = 'pipeline_code/db_cbs_tests/'
        names = 'gen_p10_n8_9_unicycle_sphere.yaml'
            ...
            with open(filename, 'r') as file:
                ...
                for given_obs in filedata['environment']['obstacles']:
                    obs.append(RectangleObstacle2D(x=given_obs['center'][0], 
                                                   ...
                                                   w=given_obs['size'][0], 
                                                   h=given_obs['size'][1]))
                this_env[TestPipelineDbaRandom.OBS_KEY] = obs
                ...
                this_env[TestPipelineDbaRandom.STARTS_KEY] = starts
                this_env[TestPipelineDbaRandom.GOALS_KEY] = goals 
    ...
    def get_starts_goals(self, seed, agents):
        ...
        possible_starts = self.envs[self.env_counter][TestPipelineDbaRandom.STARTS_KEY]
        possible_goals = self.envs[self.env_counter][TestPipelineDbaRandom.GOALS_KEY]
    ...
    def get_env_parms(self, seed): 
        ...
        obs = self.envs[self.env_counter][TestPipelineDbaRandom.OBS_KEY]
        ...
        MultiRRTPrinter.print_rrt_env('check_' + str(self.num_agents) + '_' + str(self.env_counter) + '.png',
...
for num_agents in [8]:
    ...
    tp = TestPipelineDbaRandom(test_classes, agent_builders, test_rounds=50, num_agents=num_agents, master_seed=num_agents*100+11)
    tp.run()
    tp.print_stats(filename="paper_results/dbcbs_8_9only", plots=True)
