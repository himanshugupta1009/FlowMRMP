...
class TestPipelineDbcbsFile(TestPipeline):
    OBS_KEY = 'obs'
    STARTS_KEY = 'starts'
    GOALS_KEY = 'goals'

    def __init__(self, test_classes, agent_builders, envfile, test_rounds=10, master_seed=42):
        """
        ...
        files = glob.glob(os.path.join(path, envfile))
        ...
        with open(filename, 'r') as file:
            ...
            for given_obs in filedata['environment']['obstacles']:
                obs.append(RectangleObstacle2D(x=given_obs['center'][0], 
                                                ...
                                                w=given_obs['size'][0], 
                                                h=given_obs['size'][1]))
            this_env[TestPipelineDbcbsFile.OBS_KEY] = obs
            ...
            this_env[TestPipelineDbcbsFile.STARTS_KEY] = starts
            this_env[TestPipelineDbcbsFile.GOALS_KEY] = goals 

            num_agents = len(starts)

            self.envs.append(this_env)  
        
        super().__init__(test_classes, agent_builders, test_rounds, num_agents, env_width=10, env_bredth=10, master_seed=master_seed)
    ...
    def get_starts_goals(self, seed, agents):
        ...
        possible_starts = self.envs[self.env_counter][TestPipelineDbcbsFile.STARTS_KEY]
        possible_goals = self.envs[self.env_counter][TestPipelineDbcbsFile.GOALS_KEY]
...
def get_repeatable_integer_hash(input_string):
    """
    Generates a repeatable integer hash from a string using SHA256.

    Args:
        input_string: The string to hash.

    Returns:
        A large integer representing the hash of the input string.
    """
...
for filename in filenames:
    ...
    tp = TestPipelineDbcbsFile(test_classes, agent_builders, filename, test_rounds=10, 
                               master_seed=abs(get_repeatable_integer_hash(filename)))
