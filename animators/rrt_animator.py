...
class RRTAnimator(RRT):
    def __init__(self, * , start, goal, goal_radius, env, agent,
                    ...
                    udf_seed = 77):
        """
        Initiate a new RRT planner with interactive rendering. 
    ...
    def plot_tree(self):
        """
        Display the environment with the current tree in a window    
        """
        # set up window
        self.ax.clear()
        ...
        if(len(nodes) > 1):
            # if this isn't the first node, print the path to it
    ...
    def sample_random_point(self):
        """
        Gets a new random point

        Returns:
            agent_state_type: valid random point in the env
        """
        r = self.rng.uniform(0, 1)
        if r < 0.1:
            random_point = self.goal
        else:            
            random_point = self.get_random_point(self.env, 
...
path_ids, path_states, controls, timesteps = vis_eb_rrt.plan_path()
