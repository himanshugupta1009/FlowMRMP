...
agents = [get_unicycle_agent(agent_id) for agent_id in range(num_agents)]

edge_bundle_file_location = 'edge_bundles_unclamped/eb_unicycle_dbCBS_kinodynamic_TI_edges_100000.npz'
edge_bundles = []
eb_kd_trees = []
for agent in agents:
    edge_bundle, kd_tree = load_kino_ti_edge_bundle(agent, edge_bundle_file_location)
    edge_bundles.append(edge_bundle)
    eb_kd_trees.append(kd_tree)

isvalid_funcs = [UniCycle.is_new_node_valid for _ in agents]
cost_funcs = [UniCycle.get_cost for _ in agents]
random_pt_funcs = [UniCycle.get_random_point for _ in agents]
reached_goal_funcs = [UniCycle.agent_reached_goal for _ in agents]
...
print('Planner reported planning time:', planning_time)
print('Wall planning time:', wall_time)
print('Path found:', kite_crrt.path_found)
print('Path cost:', kite_crrt.path_cost)
print('Path time:', kite_crrt.path_time)
print('Tree nodes:', len(kite_crrt.tree.nodes))

if kite_crrt.path_found:
    ...
    print('Agent path times:', kite_crrt.get_agent_path_times(high_res_paths))

    # path_rrt_nodes, states, controls, timesteps, costs = kite_crrt.get_path()
    # print('Path node ids:', path_rrt_nodes)
    # print('Path timesteps:', timesteps)

    # tcol = ['y', 'c', 'b', 'g', 'b', 'b']
    # pcol = ['xkcd:powder pink', 'xkcd:metallic blue', 'xkcd:pastel orange']
    # mprint = MultiRRTPrinter(env, kite_crrt, path_rrt_nodes, tcol, pcol, joint_states=True)
    # mprint.print_rrt('./media/kite_crrt_new_unicycle_large_env.png', print_tree=False)
